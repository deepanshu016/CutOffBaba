								...
								?>
									<table class="table table-bordered text-center ">
										...
										foreach($catids as $catid){
											...
											$R1=$this->db->select('*')->where('college_id',$collegeDetail['id'])->where('course_id',$courseid['course_id'])->where('branch_id',$branchid['branch_id'])->where('category_type',$catid['category_type'])->where('cat_id',20)->where('year',$year)->where('round_one',1)->get('tbl_cutfoff_marks_data')->result_array();
											
											$R2=$this->db->select('*')->where('college_id',$collegeDetail['id'])->where('course_id',$courseid['course_id'])->where('branch_id',$branchid['branch_id'])->where('category_type',$catid['category_type'])->where('cat_id',20)->where('year',$year)->where('round_two',1)->get('tbl_cutfoff_marks_data')->result_array(); 
											$R3=$this->db->select('*')->where('college_id',$collegeDetail['id'])->where('course_id',$courseid['course_id'])->where('branch_id',$branchid['branch_id'])->where('category_type',$catid['category_type'])->where('cat_id',20)->where('year',$year)->where('round_three',1)->get('tbl_cutfoff_marks_data')->result_array(); 
											$R4=$this->db->select('*')->where('college_id',$collegeDetail['id'])->where('course_id',$courseid['course_id'])->where('branch_id',$branchid['branch_id'])->where('category_type',$catid['category_type'])->where('cat_id',20)->where('year',$year)->where('round_four',1)->get('tbl_cutfoff_marks_data')->result_array(); 
											$R5=$this->db->select('*')->where('college_id',$collegeDetail['id'])->where('course_id',$courseid['course_id'])->where('branch_id',$branchid['branch_id'])->where('category_type',$catid['category_type'])->where('cat_id',20)->where('year',$year)->where('round_five',1)->get('tbl_cutfoff_marks_data')->result_array(); 
										?>
											<tr>
