...
Class Institute extends MY_Controller {
	...
	public function getAllInstitute()
	{
		...
		$data['categoryList'] = $this->category->getCategoriesForFeatured('tbl_course_category',array('parent_id'=>NULL));
	 	$config = array();
        $config["base_url"] = base_url() . "institute";
        $page = ($this->uri->segment(2)) ? $this->uri->segment(2) : 0;
        $config["total_rows"] = $this->institute->get_count();
        $config["per_page"] = 1;
        ...
        $data['instituteData'] = $this->institute->getPaginatedData($config["per_page"], $page);
		$this->load->view('site/instituteList',$data);
	}
}
