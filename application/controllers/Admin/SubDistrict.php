...
Class SubDistrict extends MY_Controller {
    ...
    public function importSubDistrict(){
        if ($this->is_admin_logged_in() == true) {
            $data['admin_session'] = $this->session->userdata('admin');
            $data['siteSettings'] = $this->site->singleRecord('tbl_site_settings',[]);
            $this->load->view('admin/sub_district/import',$data);
        }else{
            $this->session->set_flashdata('error','Please login first');
            return redirect('admin');
        }
    ...
    // Import CSV in DB
    public function importSubDistrictByExcel(){
                ...
                if(isset($_FILES['excel_file']['name']) && in_array($_FILES['excel_file']['type'], $file_mimes)) {
                    ...
                    if(!empty($sheetData)){
                        foreach($sheetData as $key=>$sheet){
                            $excelDatas[$key]['country'] = $sheet['country_id'];
                            $excelDatas[$key]['state'] = $sheet['state_id'];
                            $excelDatas[$key]['district'] = $sheet['district_id'];
                            $excelDatas[$key]['sub_district'] = $sheet['sub_district'];
                        }
                    }
                    $stateId = $this->master->insertBulk('tbl_sub_district',$excelDatas);
                    $response = array('status' => 'success','message' => 'Sub District imported successfully','url'=>base_url('admin/sub-district'));
                    echo json_encode($response);
                    return true;
                }
            }
