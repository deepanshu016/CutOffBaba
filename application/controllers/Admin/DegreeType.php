...
Class DegreeType extends MY_Controller {
    ...
    public function importDegreeType(){
        if ($this->is_admin_logged_in() == true) {
            $data['admin_session'] = $this->session->userdata('admin');
            $data['siteSettings'] = $this->site->singleRecord('tbl_site_settings',[]);
            $this->load->view('admin/degreetype/import',$data);
        }else{
            $this->session->set_flashdata('error','Please login first');
            return redirect('admin');
        }
    ...
    // Import CSV in DB
    public function importDegreeTypeByExcel(){
            ...
            if($ext[1] === 'csv'){
                if(($handle = fopen($tmpName, 'r')) !== FALSE) {
                    ...
                    while(($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
                        ...
                        if ($row>0) {
                            $impdata['degreetype']=$data[1];
                            $id=$data[0];
                            if($id==""){
                                $this->db->insert('tbl_degree_type',$impdata);
                            }else{
                                $this->db->where('id',$id)->update('tbl_degree_type',$impdata);
                            }
                        }
                        $row++;
                     }
                     fclose($handle);
                    $response = array('status' => 'success','message' => 'Degree Type Data imported successfully','url'=>base_url('admin/degreetype'));
                    echo json_encode($response);
                    return true;
                }else{
