...
Class ImageGallery extends MY_Controller {
	...
	public function __construct() {
   	 	parent::__construct();
   	 	$this->load->model('ImageGalleryModel','imagegallery');
	...
	public function index()
	{
		if ($this->is_admin_logged_in() == true) {
			...
			$data['galleryList'] = $this->imagegallery->getRecords('tbl_image_gallery');
			$this->load->view('admin/imagegallery/list',$data);
		}else{
	...
	public function add(){
		if ($this->is_admin_logged_in() == true) {
			$data['admin_session'] = $this->session->userdata('admin');
            $data['siteSettings'] = $this->site->singleRecord('tbl_site_settings',[]); 
			$this->load->view('admin/imagegallery/add-edit',$data);
		}else{
			$this->session->set_flashdata('error','Please login first');
			return redirect('admin');
		}
    ...
    public function editGallery($id,$slug){
        if ($this->is_admin_logged_in() == true) {
            $data['admin_session'] = $this->session->userdata('admin');
            $data['siteSettings'] = $this->site->singleRecord('tbl_site_settings',[]); 
            $data['singleGallery'] = $this->imagegallery->singleRecord('tbl_image_gallery',array('id'=>$id,'slug'=>$slug));
            $this->load->view('admin/imagegallery/add-edit',$data);
        }else{
            $this->session->set_flashdata('error','Please login first');
            return redirect('admin');
	...
	public function saveGallery(){
        ...
        if ($this->form_validation->run()) {
            $this->db->trans_start();
            $config['upload_path']  = 'assets/uploads/image_gallery/thumbnail';
            ...
            $result = $this->imagegallery->insert('tbl_image_gallery',$data);
            if(isset($_FILES['media_images']['name'])){
                $this->images->deleteRecord('tbl_media_images',array('media_id'=>$result));
                $uploadedImages = $this->uploadMultipleFiles($_FILES['media_images'],'media_images','Image Gallery',$result,'image_gallery/');
                $this->images->insert('tbl_media_images',$uploadedImages);
            }
            $this->db->trans_complete();
            if($result > 0){

                $response = array('status' => 'success','message'=> 'Image Gallery added successfully','url'=>base_url('admin/image-gallery'));
    ...
    public function updateGallery(){
        ...
        if ($this->form_validation->run()) {
            ...
            $result = $this->imagegallery->updateRecord('tbl_image_gallery',array('id'=>$this->input->post('gallery_id')),$data);
            if(isset($_FILES['media_images']['name'])){
                $uploadedImages = $this->uploadMultipleFiles($_FILES['media_images'],'media_images','Image Gallery',$this->input->post('gallery_id'),'image_gallery/');
                $this->images->insert('tbl_media_images',$uploadedImages);
            }
            $response = array('status' => 'success','message'=> 'Image Gallery updated successfully','url'=>base_url('admin/image-gallery'));
    		echo json_encode($response);
            return false;
        }else{
             $response = array(
    ...
    public function deleteGallery(){
        if ($this->is_admin_logged_in() == true) {
            ...
            $result = $this->imagegallery->deleteRecord('tbl_image_gallery',array('id'=>$id));
            if($result > 0){
                $response = array('status' => 'success','message' => 'Image Gallery deleted successfully','url'=>base_url('admin/image-gallery'));
            }else{
    ...
    public function deleteGalleryImages(){
        if ($this->is_admin_logged_in() == true) {
            ...
            if($result > 0){
                $response = array('status' => 'success','message' => 'Image deleted successfully','url'=>base_url('admin/image-gallery'));
            }else{
                $response = array('status' => 'errors','message' => 'Something went wrong !!!','url'=>'');
            }
        }else{
