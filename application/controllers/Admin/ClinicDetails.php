            ...
            if($ext[1] === 'csv'){
                if(($handle = fopen($tmpName, 'r')) !== FALSE) {
                    ...
                    while(($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
                        ...
                        if ($row>0) {
                            $impdata['details']=$data[1];
                            $id=$data[0];
                            if($id==""){
                                $this->db->insert('tbl_clinic_details',$impdata);
                            }else{
                                $this->db->where('id',$id)->update('tbl_clinic_details',$impdata);
                            }
                        }
                        $row++;
                     }
                     fclose($handle);
                    $response = array('status' => 'success','message' => 'Clinic Data   imported successfully','url'=>base_url('admin/clinicdetails'));
                    echo json_encode($response);
                    return true;
