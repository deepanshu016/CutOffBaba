...
class SubjectModel extends CI_Model {
	...
	function getRecordsOrderWise($table = '', $condition=[]){
		if(empty($condition)){
			return $this->db->order_by('show_order','ASC')->select('*')->get($table)->result_array();
		}else{
			return $this->db->order_by('show_order','ASC')->get_where($table,$condition)->result_array();
		}
	}
