...
Class Location extends MY_Controller {
    ...
    public function index()
    {
        if ($this->is_admin_logged_in() == true) {
            ...
            $data['bannerList'] = $this->master->getRecords('tbl_banner');
    ...
    public function add(){
        if ($this->is_admin_logged_in() == true) {
            ...
            $this->load->view('admin/location/add-edit',$data);
        }else{
            $this->session->set_flashdata('error','Please login first');
            return redirect('admin');
        }
    ...
    public function editBanner($id,$slug){
        if ($this->is_admin_logged_in() == true) {
            $data['admin_session'] = $this->session->userdata('admin');
            $data['siteSettings'] = $this->site->singleRecord('tbl_site_settings',[]);
            $data['singleBanner'] = $this->master->singleRecord('tbl_banner',array('id'=>$id,'slug'=>$slug));
    ...
    public function saveBanner(){
        ...
        if ($this->form_validation->run()) {
            ...
            $result = $this->master->insert('tbl_banner',$data);
            if($result > 0){
